...
for variable in ["best_treelime", "method", "perturbations", "flat_or_layered", "perturbation_chance", "dist", "time", "rel_tol"]
                    ...
                    for rel_tol in possible_rel_tols
                        ...
                        if type == "layered"
                            transform!(filtered_df1, :name => (
                                x -> "TreeLIME\ndir = $(tr("UP")) \nγ ~ " .* string.(tr.(perturbation_chances)) .* "\nδ = $(tr(dist))"
                            ) => :Formatted_name)
                        else
                            transform!(filtered_df1, :name => (
                                x -> "TreeLIME\nγ ~ " .* string.(tr.(perturbation_chances)) .* "\nδ = $(tr(dist))"
                    ...
                    for perturbation_chance in perturbation_chance_local
                        ...
                        for dist in possible_dist_local
                            for rel_tol in possible_rel_tols
                                ...
                                title = "$(tr(method)) in $(tr(type)) mode\nwith γ ~ $(tr(perturbation_chance)), δ = $(tr(dist)) and relative threshold $(rel_tol)%"
                                println(title)
                                filtered_df = nothing
                                if method == "lime"
        ...
        for method in possible_methods
            for pertubation_count in possible_perturbations
                ...
                for perturbation_chance in perturbation_chance_local
                    ...
                    for dist in possible_dist_local
                        for rel_tol in possible_rel_tols
                            filename = "method=$((method)), $(pertubation_count) perturbations, perturbation_chance = $(perturbation_chance), dist = $(dist), rel_tol = $(rel_tol)"
                            title = nothing
                            if method == "lime"
                                title = "$(tr(method)) with $(pertubation_count) perturbations\n γ ~ $(tr(perturbation_chance)), δ = $(tr(dist)) and relative threshold $(rel_tol)%"
                            else
                                title = "$(tr(method))\n with $(pertubation_count) perturbations and relative threshold $(rel_tol)%"
                            end
                            println(title)
        ...
        for method in possible_methods
            for pertubation_count in possible_perturbations
                ...
                for perturbation_chance in perturbation_chance_local
                    ...
                    for dist in possible_dist_local
                        filename = "method=$((method)), $(pertubation_count) perturbations, perturbation_chance = $(perturbation_chance), dist = $(dist)"
                        title = nothing
                        if method == "lime"
                            title = "$(tr(method)) with $(pertubation_count) perturbations\n γ ~ $(tr(perturbation_chance)), δ = $(tr(dist))"
                        else
                            title = "$(tr(method))\n with $(pertubation_count) perturbations"
                        end
                        println(title)
                        filtered_df1 = nothing
                        filtered_df2 = nothing
        ...
        for pertubation_count in possible_perturbations
            for type in possible_type
                ...
                for direction in possible_direction_local
                    ...
                    for dist in possible_dist_local
                        for rel_tol in possible_rel_tols
                            filename = "$(pertubation_count) perturbations,type=$((type)), direction = $(direction), dist = $(dist), rel_tol = $(rel_tol)"
                            title = nothing
                            if type == "Flat"
                                title = "$(tr("lime")) in $(tr(type)) mode\nwith $(pertubation_count) perturbations, δ = $(tr(dist)) and relative tolerance $(rel_tol)%"
                            else
    ...
    elseif variable == "dist"
        for pertubation_count in possible_perturbations
            for perturbation_chance in [1.2]
                for type in possible_type
                    possible_direction_local = possible_direction
                    if type == "Flat"
                        possible_direction_local = ["UP"]
