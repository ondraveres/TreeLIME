...
function getVariants()
    ...
    uninformative = [:Flat_Gadd, :Flat_Garr, :Flat_Garrft, :LbyL_Gadd, :LbyL_Garr, :LbyL_Garrft]#[:Flat_Gadd, :Flat_Garr, :Flat_Garrft, :LbyL_Gadd, :LbyL_Garr, :LbyL_Garrft]
    vcat(
        #("const", :Flat_Gadd),
        #("const", :LbyL_Gadd),
...
function getexplainer(name; sch=nothing, extractor=nothing)
    ...
    elseif startswith(name, "l2-distance")
        split_name = split(name, "_")
        perturbation_count = parse(Float64, split_name[2])
        return ExplainMill.DafExplainer(perturbation_count, true, false, extractor)
